// Invokes wipeData.ts (packages/edd-workbench-core/src/wipeData.ts) against
// a real deployed environment via ECS RunTask — same "no path from a
// laptop to RDS" reasoning as run-migrations.mjs/run-reindex.mjs, reusing
// the migrate task definition (the only one with DB *owner* credentials
// already securely wired) with its command overridden instead of a task
// definition of its own.
//
// DESTRUCTIVE: deletes every matter, document, tag, and every other real
// data row in the target environment. No confirmation prompt lives here —
// this script assumes whoever is running it has already decided to do
// this; the confirmation belongs in the conversation/process that led here,
...
if (!stackName) {
  console.error("Usage: node scripts/run-wipe-data.mjs <stack-name>");
  process.exit(1);
}
...
async function run() {
  ...
  console.log("Starting wipe-data task...");
        ...
        containerOverrides: [
          {
            ...
            name: "migrate",
            command: ["sh", "-c", "node_modules/.bin/tsx packages/edd-workbench-core/src/wipeData.ts"],
          },
        ],
      },
    }),
  ...
  for (;;) {
    ...
    if (task?.lastStatus === "STOPPED") {
      ...
      if (exitCode === 0) {
        console.log("Wipe-data task completed successfully.");
        return;
      }
      throw new Error(`Wipe-data task exited with code ${exitCode}: ${task.stoppedReason}`);
    }
