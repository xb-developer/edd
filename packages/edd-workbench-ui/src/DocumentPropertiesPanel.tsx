...
export function DocumentPropertiesPanel({ document }: DocumentPropertiesPanelProps) {
  ...
  return (
    <div className="mb-3">
      <span className="mb-1.5 inline-block rounded-[3px] bg-navy px-2 py-0.5 font-mono text-xs font-semibold text-white">{document.guid}</span>
      <div className="text-xs font-semibold break-words">{displayName(document)}</div>
      <dl className="mt-2.5 grid grid-cols-[auto_1fr] gap-x-2.5 gap-y-[5px] text-xs [&_dt]:text-ink-soft [&_dd]:m-0 [&_dd]:break-words">
        <dt>Type</dt>
