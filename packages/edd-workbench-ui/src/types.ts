...
export interface DocumentDTO {
  ...
  title: string | null;
  /** For an email, the sender's display name (or their address if no name was given) — NOT the combined "Name <address>" form. For docx/xlsx/pptx/odt/ods/odp/epub/rtf/html, the document's own Author metadata property; for pdf, its /Author Info-dictionary entry. Null for formats with no author concept (image/tiff, plain text) or where extraction found none. */
  author: string | null;
  subject: string | null;
  docDate: string | null;
  /** The source document's OWN internal last-modified metadata property (docProps/core.xml's dcterms:modified for docx/pptx/xlsx, officeparser's own reading for odt/ods/odp/epub/rtf/html, a pdf's own /ModDate|/CreationDate) — NOT fileModifiedAt, which is the uploaded file's browser-reported mtime and is never set at all for an attachment. Null wherever the property isn't available for a format (email — see docDate instead — legacy .doc, image/tiff, text/other) or extraction found none. */
